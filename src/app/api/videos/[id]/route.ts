...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  return NextResponse.json({ ...data, artists: videoArtists || [] });
}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from("videos").update(body).eq("id", params.id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });

  // Update artist links if provided
  if (artist_ids !== undefined) {
    await supabase.from("video_artists").delete().eq("video_id", params.id);
