...
export default function EditReleasePage() {
  ...
  async function fetchThumbnails() {
    ...
    if (urls.length === 0) {
      toast("Add some streaming links first", "info");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.thumbnails.length === 0) {
          toast("No thumbnails found from the links", "info");
        }
      }
    } catch {
      toast("Failed to fetch thumbnails", "error");
    }
  ...
  async function handleDelete() {
    ...
    if (res.ok) { toast("Deleted"); router.push("/admin/releases"); }
  }
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/admin/releases" className="inline-flex items-center gap-2 text-sm font-body text-cobalt/40 hover:text-cobalt mb-6">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <section className="card-editorial p-6 space-y-4">
          ...
          <Select
            id="releaseType" label="Release Type" value={releaseType}
        ...
        <section className="card-editorial p-6 space-y-4">
            ...
            <label className="flex flex-col items-center justify-center w-48 h-48 rounded-2xl border-2 border-dashed border-cobalt/15 bg-cobalt/5 cursor-pointer hover:border-accent hover:bg-accent/5 transition-all">
              {uploadingCover ? (
                ...
                <>
                  ...
                  <span className="text-xs font-display font-bold uppercase tracking-wider text-cobalt/30">Upload Cover</span>
                </>
              )}
          ...
          <div className="pt-2">
            <Button
              ...
              loading={fetchingThumbs}
            >
              <ImageIcon className="w-4 h-4 mr-2" />
              Auto-fetch from streaming links
            </Button>
            <p className="text-[10px] text-cobalt/30 font-body mt-1">
              Fetches album art from Spotify, Apple Music, YouTube, SoundCloud links below.
            </p>
          </div>

          {/* Fetched thumbnails to choose from */}
          {fetchedThumbs.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-display font-bold uppercase tracking-widest text-cobalt/60">
                Choose a cover ({fetchedThumbs.length} found)
              </p>
              <div className="grid grid-cols-3 gap-3">
                {fetchedThumbs.map((thumb, i) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setCoverArtUrl(thumb.image);
                      setFetchedThumbs([]);
                      toast(`Using cover from ${thumb.source}`);
                    }}
                    className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all hover:scale-105 ${
                      coverArtUrl === thumb.image
                        ? "border-accent shadow-lg"
                        : "border-cobalt/10 hover:border-accent/50"
                    }`}
                  >
        ...
        <section className="card-editorial p-6 space-y-4">
          ...
          <p className="text-xs text-cobalt/30 font-body">
            Add links to where this release can be streamed or purchased.
          </p>
          {STREAMING_PLATFORMS.map((p) => (
            <Input
              key={p.key}
              id={`release-stream-${p.key}`}
              label={p.label}
