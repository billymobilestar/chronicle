...
export default function ArtistEditVideoPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (res.ok) { toast(isNew ? "Video added!" : "Updated!"); router.push("/artist-videos"); }
  ...
  async function handleDelete() {
    ...
    if (res.ok) { toast("Deleted"); router.push("/artist-videos"); }
  }
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/artist-videos" className="inline-flex items-center gap-2 text-sm font-body text-cobalt/40 hover:text-cobalt mb-6">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        <section className="card-editorial p-6 space-y-4">
          ...
          <Input id="youtubeUrl" label="YouTube URL" value={youtubeUrl} onChange={(e) => setYoutubeUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." required />
          {ytId && (
            <div className="aspect-video rounded-2xl overflow-hidden">
              <iframe src={`https://www.youtube.com/embed/${ytId}`} allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full" />
            </div>
          )}
        ...
        <section className="card-editorial p-6 space-y-4">
          ...
          <p className="text-xs text-cobalt/30 font-body">Tag all artists in this video. It shows on each tagged artist&apos;s profile.</p>
          <ArtistMultiSelect selectedIds={artistIds} onChange={setArtistIds} />
        </section>
